  ...
  skills: (
    <section id="skills">
      <div className="flex min-h-0 flex-col gap-y-4">
        ...
        <BlurFade delay={BLUR_FADE_DELAY * 9}>
          <h3 className="text-lg font-bold">{DATA.sections.skills.subheading}</h3>
        </BlurFade>
        <div className="flex flex-wrap gap-2">
          {DATA.skillsgeneral.map((skillsgeneral, id) => (
            <BlurFade key={skillsgeneral.name} delay={BLUR_FADE_DELAY * 10 + id * 0.05}>
              <div className="border bg-background border-border ring-2 ring-border/20 rounded-xl h-8 w-fit px-4 flex items-center gap-2">
                {skillsgeneral.icon && <skillsgeneral.icon className="size-4 rounded overflow-hidden object-contain" />}
                <span className="text-foreground text-sm font-medium">{skillsgeneral.name}</span>
              </div>
            </BlurFade>
          ))}
        </div>
        <BlurFade delay={BLUR_FADE_DELAY * 9}>
          <h3 className="text-lg font-bold">{DATA.sections.skills.subheadingtwo}</h3>
        </BlurFade>
        <div className="flex flex-wrap gap-2">
